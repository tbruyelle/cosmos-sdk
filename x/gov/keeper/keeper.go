...
func NewKeeper(
	...
	k := &Keeper{
		...
		QuorumCheckQueue:       collections.NewMap(sb, types.QuorumCheckQueuePrefix, "quorum_check_queue", collections.PairKeyCodec(sdk.TimeKey, collections.Uint64Key), codec.CollValue[v1.QuorumCheckQueueEntry](cdc)), // sdk.TimeKey is needed to retain state compatibility
	}
	schema, err := sb.Build()
	if err != nil {
		panic(err)
